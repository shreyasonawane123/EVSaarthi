...
const ReviewsPage = () => {
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_GATEWAY}/api/admin/reviews`, {
      ...
      if (msg && msg.includes("https://console.firebase.google.com")) {
        ...
        console.error("🔥 FIRESTORE INDEX LINK:", msg);
      } else {
        showNotify('Failed to fetch reviews: ' + (err.response?.data?.error || err.message), 'error');
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-20 text-center border-2 border-dashed border-gray-200">
          <p className="text-gray-400 font-bold">No reviews found in the system.</p>
        </div>
      ) : (
